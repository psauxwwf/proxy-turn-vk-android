...
Если не нужен деплой через Android-приложение, сервер можно поднять напрямую через Docker Compose.

1. Скопируйте на сервер `docker-compose.yaml` из репозитория.
2. Создайте рядом файл `.env` и запишите в него главный пароль WRAP:

```env
WDTT_MAIN_PASSWORD=your-strong-password
```

3. Выполните загрузку образа:
...
5. При необходимости проверьте логи:

```sh
docker compose logs -f wdtt-server
```

По умолчанию compose поднимает сервер с портами `56000/udp` для DTLS и `56001/udp` для WireGuard и сохраняет данные в каталоге `./wdtt`.
